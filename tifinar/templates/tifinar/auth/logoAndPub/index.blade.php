                ...
                <div class="book" dir="{{ $direction }}">
                    <a href="{{ route('books.show', $book) }}">
                        <figure>
                            ...
                            <figcaption class="edit-book-title">{{ $book->title }}</figcaption>
                        </figure>
                        <hr>
                    </a>
                    <div class="listes-contenu-VD">
                        <a title="معاينة الكتاب" href="{{ route('books.show', $book) }}" class="btn btn-secondary">
                        ...
                        <a title="تعديل الكتاب" href="{{ route('books.edit', $book) }}" class="btn btn-primary">
                            <i class="fas fa-edit"></i>
                        </a>

                        <form action="{{ route('books.update', $book) }}" method="POST" style="display:inline;">
                            @csrf
                            @method('PATCH')
                            <button title="{{ $isPublic ? 'الكتاب يظهر للعامة، أنقر لجعله يظهر للمدراء فقط' : 'يظهر للمدراء فقط، أنقر لجعله يظهر للعامة' }}" 
                                    class="btn {{ $isPublic ? 'btn-success' : 'btn-warning' }}" 
                                    type="submit" name="visibility" value="{{ $isPublic ? 'restricted' : 'public' }}">
