...
def article_detail(request, slug):
    ...
    next_obj = (
        articles.objects.filter(art_id__gt=article.art_id).order_by('art_id').first() or
        articles.objects.filter(art_id__lt=article.art_id).order_by('-art_id').first()
    )

    # المقالات ذات الصلة مع مراعاة المستوى التعليمي والعمر (إذا كان المستخدم مسجل الدخول)
    base_query = articles.objects.exclude(slug=slug)
