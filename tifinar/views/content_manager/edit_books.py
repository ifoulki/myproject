...
def edit_book(request, content_type, slug):
    ...
    if content_type == 'books':
        ...
        print(f"طول العنوان: {len(content.title)}")
        ...
        print(f"طول العنوان بعد التطبيع: {len(normalized_book_title)}")
        ...
        for comment in all_comments:
            ...
            contains_match = normalized_book_title in normalized_comment_title
            comment_contains_book = normalized_comment_title in normalized_book_title
            
            print(f"التعليق {comment.cmt_id}: '{comment.page_title}'")
            print(f"  تطبيع: '{normalized_comment_title}'")
            print(f"  تطابق تام؟ {exact_match}")
            print(f"  التعليق يحتوي على عنوان الكتاب؟ {contains_match}")
            print(f"  العنوان يحتوي على التعليق؟ {comment_contains_book}")
            
            if exact_match or contains_match or comment_contains_book:
        ...
        # إذا لم توجد تعليقات، جرب جلب التعليق المحدد يدويًا (cmt_id: 8)
        if not content_comments:
            print("محاولة جلب التعليق المحدد (cmt_id: 8)...")
            try:
                specific_comment = comments.objects.get(cmt_id=8)
                content_comments = [specific_comment]
                print(f"تم تحميل التعليق المحدد: {specific_comment.page_title}")
            except comments.DoesNotExist:
                print("التعليق المحدد غير موجود")
    ...
    context = {
        ...
        'comment_count': len(content_comments) if content_type == 'books' else 0  # التصحيح هنا: استخدام len() بدلاً من count()
    }
